...
class timed_test_execution //extends pts_test_execution
{
	public static function run_test(&$test_run_manager, &$test_run_request, $time_to_execute_sec)
	{
		...
		if($time_to_execute_sec != false)
		{
			$time_test_should_end = $time_test_start + $time_to_execute_sec;
		}
		else
		{
			$time_test_should_end = false;
		}
		...
		for($i = 0, $abort_testing = false, $time_test_start_actual = time(), $defined_times_to_run = $times_to_run; $i < $times_to_run && !$abort_testing; $i++)
		{
			...
			if($is_expected_last_run && $test_run_request->test_result_buffer->get_count() > floor(($i - 2) / 2))
			{
				...
				else if($time_test_should_end > time())
				{
					echo " [Time left: " . ($time_test_should_end - time()) . " Seconds]";
					$increase_run_count = 1;
				}
